...
function ReflectionPage() {
    const [showModal, setShowModal] = useState(true); // 🚨 Always open SportSelectionModal initially
    const [selectedSport, setSelectedSport] = useState(null);
    const [scoreSummary, setScoreSummary] = useState(null);

    const [answers, dispatch] = useReducer(answersReducer, {}, () => {
        return {}; // 🚨 Start blank by default
    });
    ...
    const handleSelectSport = (sport) => {
        ...
        dispatch({ type: 'RESET' }); // 🚨 RESET ALL ANSWERS
        setShowModal(false); // Close the sport selection modal
    };

    const handleResetSport = () => {
        localStorage.removeItem('selectedSport');
        dispatch({ type: 'RESET' }); // 🚨 Clear answers too
        setShowModal(true); // Open sport selection modal again
