...
const FitnessSnapshot = ({ workouts }) => {
    const getMetric = (activityType) => {
        ...
        let total = 0;
        if (activityType === 'Run') {
            total = entries.reduce((sum, w) => sum + Number(w.miles || 0), 0);
            ...
            total = entries.reduce((sum, w) => sum + Number(w.duration || 0), 0);
    ...
    return (
        <div className="grid grid-cols-2 gap-4 mb-8">
            {metrics.map((metric) => (
                <div key={metric.name} className="bg-white dark:bg-gray-800 shadow rounded-xl p-4 text-center">
